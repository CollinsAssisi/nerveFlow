...
#    output = sess.run(c) 
\end{minted}
By specifying the computation graph we also specify the dependcies among the nodes. One can thus split the graph into smaller chunks or sub-graphs that can be independently computed by different devices that coordinate with each other. This makes it possible to develop programs that are device independent and scalable across CPUs, GPUs , TPUs and clusters of servers. GPUs and TPUs are relatively recent additions to the computing 

\subsection*{Advantages of GPU/TPU vs CPU?}
NVIDIA introduced its first Graphical Processing Unit (or GPU), the GE XForce GTE ??, in 1999. Until then all calculations and graphical rendering was done using the CPU. 
